...
- [x] **Visual ingest lives in the Knowledge Vault too** — one component (`components/spark/AdIngest.tsx`) serves both the Creative page and the Vault. Every read design is banked with its full `VisualDNA` in chunk metadata, and `lib/visual-library.ts` ranks banked designs against a brief so the Reactor auto-pulls the best-fitting proven design when no reference is attached (`designOnly` — direction, not a clone order)
