  ...
  const handleGenerate = async (b: CampaignBrief) => {
    ...
    try {
      ...
      const [claudeRes, openaiRes] = await Promise.allSettled([
        postJson('/api/generate-copy', { brief: b }),
        postJson('/api/generate-copy-openai', { brief: b }),
      ])
      ...
      const imagePrompt =
        primary.imagePrompt ||
        `Professional advertising photograph for Summit Build Co — ${b.angle}`
    ...
    <main className="min-h-screen bg-[#0a0a0a] text-white">
      <header className="border-b border-white/10 px-8 py-5">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div>
            <h1 className="text-xl font-semibold tracking-tight">Summit Build Co</h1>
            <p className="text-xs text-white/40 mt-0.5">AI Creative System — Demo</p>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></div>
            <span className="text-xs text-white/40">Brand memory loaded</span>
          </div>
        </div>
      </header>
            ...
            {appState === 'idle' && (
              <div className="rounded-xl border border-white/10 bg-white/[0.02] p-12 text-center">
                <p className="text-white/30 text-sm">
                  Enter a campaign brief to generate your first ad creative
                </p>
              </div>
            )}
