  ...
  return (
    <span className={cn('group/tip relative inline-flex items-center', className)}>
      ...
      <span
        role="tooltip"
        className={cn(
          'pointer-events-none absolute bottom-full z-30 mb-2 w-60 rounded-lg border border-border bg-background/95 p-2.5 text-[11px] leading-relaxed text-white/70 opacity-0 shadow-panel backdrop-blur-md transition-opacity duration-150 group-hover/tip:opacity-100 group-focus-within/tip:opacity-100',
          align === 'right' ? 'right-0' : 'left-0',
        )}
      >
        {label && (
          <span className="mb-0.5 block text-[10px] font-semibold uppercase tracking-[0.14em] text-white/45">
            {label}
          </span>
        )}
        {children}
...
export function EvidenceLine({ items }: { items: (string | null | undefined)[] }) {
  ...
  return (
    <p className="flex flex-wrap items-center gap-x-2 gap-y-1 text-[11px] tabular text-white/45">
      {shown.map((item, i) => (
